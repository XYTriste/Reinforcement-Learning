...
\tag{3}
$$
其中矩阵的每一行数据表示从某一个状态到所有$n$个状态的转移概率值，每一行的这些值的和应该为1。说明针对某一状态所有可能发生的事情的总概率应该为1。<font color="red">每一列则说明当下一个状态为$j$时，当前状态是某个状态的概率。</font>
例如，$P_{13} = 0.5$说明了由状态1转移到状态3的概率为0.5。而$P_{51} = 0.3$则说明当前状态为状态5时，下一个状态是状态1的概率为0.3。
### 马尔科夫奖励过程
