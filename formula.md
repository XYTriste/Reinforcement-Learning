...
更具体的说，针对一个状态$s$，个体通过行为策略产生一个行为$a$，执行该行为进而产生一个状态-行为对$(s,a)$，并基于此获得一个即时奖励$R_s^a$并进入状态$s^{'}$。在新的状态$s^{'}$下，个体遵循策略产生一个新的行为$a^{'}$。但是此时个体并不立即执行该行为，而是通过行为价值函数得到$q_\pi(s,a)$的价值。