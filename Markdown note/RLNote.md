...
\end{align}
$$
其中，$\nabla_{\theta}log\pi_{\theta}$被称为分值函数，我们需知:<font color="red">对于任何可微的策略函数$\pi_{\theta}(s,a)$以及三种策略目标函数$J=J_1,J_{avgV}$和$J_{avgR}$的任意一种来说，策略目标函数的梯度都可以写成使用分值函数表示的形式:</font>
$$
\nabla_{\theta}J(\theta)=\Bbb E_{\pi_{\theta}}[\nabla_{\theta}log\pi_{\theta}(s,a)Q_{\pi_{\theta}}(s,a)]
$$
在这个公式中，log是来自于策略梯度定理的推导过程。
策略梯度定理告诉我们，策略梯度可以被表示为分值函数乘以策略的梯度，即：
$$
\nabla_{\theta}J(\theta)=\Bbb E_{\pi_{\theta}}[\nabla_{\theta}log\pi_{\theta}(s,a)Q_{\pi_{\theta}}(s,a)]
$$
<font color="red">其中，$\log \pi_{\theta}(s,a)$ 是策略在状态 $s$ 选择行为 $a$ 的概率的自然对数。</font>

这里使用log函数是为了将策略的概率表达式转化为可微分形式。对于很多概率分布而言，我们可以用自然对数对其取对数，然后将它转化为指数函数的形式，使得我们可以方便地对它求导。因此，策略梯度算法中使用了log函数，以便将概率转化为可微分的形式。

## Softmax策略
Softmax策略是一个应用于离散行为空间的常用策略，该策略使用描述状态和行为的特征$\phi(s,a)$与参数$\theta$的线性组合来权衡一个行为发生的概率:
$$
\pi_{\theta}\propto e^{\phi(s,a)^{T}\theta}
$$
> $\propto$表示"与...成比例"或"正比于..."

相应的分值函数为:
$$
\nabla_{\theta}log\pi_{\theta}(s,a)=\phi(s,a)-\Bbb E_{\pi_{\theta}}[\phi(s, ·)]
$$
>这个公式是策略梯度算法的重要公式之一，用于计算策略梯度。具体地，$\pi_{\theta}(s,a)$是在状态$s$下选择动作$a$的概率，$log\pi_{\theta}(s,a)$表示对数概率，$\theta$是策略函数的参数，$\phi(s,a)$是一些特征函数，通常用来表示状态和动作的特征。
公式右边的第一项 $\phi(s,a)$ 表示在当前状态下，选择某个动作的特征，是与参数 $\theta$ 无关的。而右边的第二项 $\Bbb E_{\pi_{\theta}}[\phi(s, ·)]$ 表示对于当前策略 $\pi_{\theta}$ ，在状态 $s$ 下选择所有动作的特征的期望值，是与选择的动作 $a$ 无关的。
因此，公式的意义是计算在状态 $s$ 下选择动作 $a$ 对策略的梯度，该梯度由选择该动作的特征值减去所有动作特征值的期望值得到。
这个等式告诉我们，选择动作$a$的特征值$\phi(s,a)$减去所有动作的特征值$\Bbb E_{\pi_{\theta}}[\phi(s, ·)]$的期望值，就是分值函数的梯度。这是因为，对于一个特定的状态和动作，$\phi(s,a)$可以被看作是一个重要性权重，它表示了选择该动作对策略梯度的贡献。而$\Bbb E_{\pi_{\theta}}[\phi(s, ·)]$则是所有可能动作的平均贡献，它可以看作是一种基准，用于衡量选择该动作的相对重要性。因此，选择动作的特征值减去所有动作特征值的期望值。


## 高斯策略
高斯策略是应用于连续行为空间的一种策略，该策略对应的行为从高斯分布中产生。其均值$\mu(s)=\phi(s)^{T}\theta$。高斯策略对应的分值函数为:
$$
\nabla_{\theta}log\pi_{\theta}(s,a)=\frac{(a-\mu(s))\phi(s)}{\sigma^{2}}
$$
对于连续行为空间的每一个行为特征，由策略$\pi(\theta)$产生的行为对应的该特征分量都服从一个高斯分布，该分布中采样得到一个具体的行为分量，多个行为分量整体形成一个行为，再根据采样得到的不同行为对应于不同的奖励。参数$\theta$的调整方向是用一个新的高斯分布去拟合使得那些得到正向奖励的行为值和负向奖励的行为值的相反数形成相同的采样结果。

## Actor-Critic算法
该算法包含了一个策略函数和一个行为价值函数。策略函数充当Actor，生成行为与环境交互。而行为价值函数充当Critic，指导Actor的后续行为动作。Critic的行为价值函数是基于策略$\pi_{\theta}$的一个近似:
$$
...
\end{align}
$$
由于书中描述的流程不清晰，这部分笔记暂时到这.

---

## DDPG算法
DDPG算法中的Actor和Critic都使用深度神经网络来建立近似函数。由于该算法可以直接从Actor的策略生成确定的行为而不需要依据行为的概率分布进行采样，因此称为确定性策略。算法在训练阶段通过在确定性的行为基础上增加一个噪声函数，实现在确定性行为周围的小幅度探索。算法还为Actor和Critic网络各备份了一套参数来计算行为价值的期望值来更稳定的更新Critic的指导水平，使用备份参数的网络称为目标网络。而主网络中的Actor和Critic每训练一轮就更新一次。使用这样的方式的原因和DQN算法的原理类似，都是为了降低时间上连续的样本导致的训练效果问题。这四个网络的具体使用情景为:
1. Actor网络:根据当前状态$s_0$生成的探索或不探索的具体行为$a_0$
2. Target Actor网络:根据环境给出的后续状态$s_1$生成预估价值用到的$a_1$
3. Critic网络:计算状态$s_0$和生成的$a_0$对应的行为价值
4. Target Critic网络:根据后续状态$s_1, a_1$生成用来计算目标价值$y=Q(s_0,a_0)$的$Q^{'}(s_1,a_1)$
