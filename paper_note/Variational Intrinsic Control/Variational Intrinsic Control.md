...
>
> 如果在给定选项$\Omega$的情况下，某个最终状态$s_f$的概率$p^J(s_f|s_0,\Omega)$较高，那么它对应的信息熵就会较小，意味着确定该最终状态所需要的信息量较少，我们能够较好的控制该状态。相反，如果某个最终状态$s_f$的概率较低，它所对应的信息熵就会较大。意味着确定该最终状态所需要的信息量较多，我们对最终状态的控制程度较低。
>
> 基于上述内容，我们得到了某个选项$\Omega$对于最终状态的控制程度，使用信息熵进行衡量。通过对于所有的选项$\Omega$和最终状态$s_f$按照选项被选择的概率以及最终状态的概率分布进行加权求和。我们就得到了最终的对整个环境的控制程度。
>
> 现在，我们得到了熵$H(s_f)$来确定我们对一个选项的控制程度，得到了给定选项下最终状态的控制程度。二者相减就得到了选项$\Omega$和最终状态$s_f$之间的互信息。互信息是信息论中用来衡量两个随机变量之间相互依赖程度的量。在这里，它用来衡量选项$\Omega$和最终状态$s_f$在给定初始化状态$s_0$的条件下的相关性。通过优化互信息，我们能够找到使得最终状态多样性最大化且控制精度最高的选项，从而实现最优的内在控制。

![image](.\mutual information.jpg)

互信息是对称的，第二行包含其反向表达式。这个表达式有一个非常直观的解释：如果我们能从最终状态推断出选项，那么我们就应该能够把它们区分开来。也就是说，如果对于两个选项$\Omega_1$和$\Omega_2$来说，在到达状态$s_{f1}$时，我们就可以推断出执行的是选项$\Omega_1$而不是$\Omega_2$。而在到达状态$s_{f2}$时，我们就可以推断执行的是选项$\Omega_2$而不是$\Omega_1$。那么这两个选项就可以称之为本质上不同的选项。我们希望最大化选项集--实现巨大的熵$p(\Omega|s_0)$。与此同hi，我们希望确保这些选项能够实现本质上不同的目标，即它们的熵值不同。也就是说可以从它们的最终状态推断出不同的目标，这就需要最大化$\log{p(\Omega|s_0,s_f)}$，其均值就是式(2)中的第二项。