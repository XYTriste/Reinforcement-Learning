...
当我们想要用这样的模板创建一个整数矩阵的实例时，我们通常会使用这样的语句:

```cpp
SquareMatrix<int, 5> m1; // 创建一个5*5的矩阵
SquareMatrix<int, 10> m2; // 创建一个10*10的矩阵
```

我们可以试想一下逆矩阵函数的操作（虽然我忘了怎么对矩阵求逆了），假设它是一个双重循环求每个行列位置上的逆，那么它必须要知道矩阵的大小，这也是为什么我们创建模板时需要`std::size_t n`的原因。

然而，如果我们将`n`作为循环变量的话，这意味着对于`m1`和`m2`而言，它们的`invert`函数在这部分是不同的（尽管只是一个循环变量的差距），编译器会为这两个对象生成不同的模板类实例，尽管这两个实例只有`invert`函数的差异。这无疑造成了代码膨胀。

既然将`n`作为模板参数会导致这样的问题，那么显然不要这么做就好了。但是对矩阵的操作绝大多数情况下都需要事先知道矩阵大小，所以无论如何我们还是需要这个`n`，因此我们改写成这样的代码：
...
class SquareMatrixBase{
	protected:
		...
		void invert(std::size_t matrixSize); //改写后的逆矩阵函数，添加了参数
};
template<typename T, std::size_t n>
...
这段代码实在是绝妙，我将逐步的对它进行解释。

首先我们将实际做逆矩阵操作的模板类改写成了模板基类，并把其`invert`函数访问属性由`public`改成了`protected`。其次，我们定义了一个类来`private`继承这个模板基类，记得之前提到过的“私有继承仅继承功能而不继承接口”吗？为了能够调用实际做逆矩阵操作的`SquareMatrixBase::invert`，我们使用了`using`声明来显露它，解决了名称隐藏的问题（条款33）。

另一个绝妙之处（其实理当如此）的地方在于，模板基类中的`invert`函数的访问属性由`public`改写为了`protected`，这样做也避免了用户直接创建一个`SquareMatrixBase`对象后不讲道理的调用`invert`函数（我们可不希望`invert`函数的参数由我们来传递，而是在矩阵创建的时候就指定），因此我们使用了“转交函数”，将实际的逆矩阵函数由派生类转交给基类进行操作。

或许会有点让人迷惑，派生类的模板参数中仍然包含了数字，这似乎意味着`SquareMatrix`这个模板类仍然会为不同的数字生成相应的目标码，也就是：
```cpp
template<typename T, std::size_t n>
class SquareMatrix: private SquareMatrixBase<T>{
    private:
    ...
    public:
    	...
    	void invert(){
            this->invert(5); // 为参数5生成这样的模板，10也是同理。
        }
};
```
...
我个人目前的理解是这是“两相权害取其轻”的结果，由于`SquareMatrix`并不提供实际的功能，因此这个类不会太长。相较于让复杂的`SquareMatrixBase`膨胀，还是让这个比较简单的类膨胀好了。

- 模板生成多个类和多个函数，所以任何模板代码都不应该与某个造成膨胀的模板参数产生相依关系。
- 因非类型模板参数而造成的代码膨胀，往往可以消除，做法是以函数参数或 class成员变量替换模板参数。
- 因类型参数而造成的代码膨胀，往往可以降低，做法是让带有完全相同二进制表述的具现类型共享实现码。

## 45. 运用成员函数模板接受所有兼容类型

C++中的指针做到了一件非常好的事，隐式转换。例如派生类指针可以隐式转换为基类指针、指向非const对象的指针可以转换为指向const对象等等。

但是究其本源，还是因为它们之间存在着一定的“关系”，例如派生类与基类之间的父子关系，非const和const之间的转换也发生在同辈或父子关系的对象之上。哪怕是C++的内置类型（例如int、double）之间也是能够隐式转换的（它们都是数值），尽管编译器大多为此发出一个警告。

现在的一个问题是，由模板类生成的类实体之间的关系是什么呢？例如，`vector<int>`和`vector<double>`之间是什么关系呢？这就好像从同一个基因中克隆出的不同人一样。本质上它们是非常相似的，但是由于拥有不同的记忆，不同的社会关系，它们又可以说是关系不深。

话说回来，从直觉上来说我们也不能说`vector<int>`和`vector<double>`毫无关系，有时候我们甚至期望将数据从其中一个实体复制到另一个实体，为此我们必须让它们具有某种关系，从而支持这样的转换。
...
我们期望`SmartPtr<Top> pt1`指针可以接受所有`Top`及其派生类的对象作为被指之物，事实上它也确实能做到。然而，如果我们未来添加了新的派生类呢？`BelowBottom`或者其他派生类，虽然从我们的示例代码中似乎不用做出任何改动就能同样进行转换。但是考虑到派生类通常需要实现额外的操作，因此`SmartPtr`的构造函数还得为这些新的派生类做出一些适当的处理来兼容。

为每一个新出现的派生类都添加一些处理显然是行不通的，为此我们需要一样东西，那就是成员函数模板：
...
上述代码的含义是，对于任何类型的`T`和任何类型的`U`，都能够根据`other`来构造一个对象`t`，`T`和`U`可以是同一模板（都是`SmartPtr<T>`模板）生成的不同具体类。

还需要解决的一个小问题是，如果`U`在继承结构中处于更高层次怎么办？换句话说，将`int`转换为`double`可能不会有问题，但是`double`转换为`int`似乎必然造成精度损失。基类指针指向派生类对象不会有问题（只要它们使用相同的接口，甚至这就是我们常做的），而派生类指针指向一个基类的对象可能引发我们绝不想看到的结果。

为此我们必须事先知道`T`和`U`的类型，从而保证不会做出我们不希望看到的转换：
```cpp
template<typename T>
class SmartPtr{
    public:
...
又是一段优美的代码！我们巧妙的运用了`T`类型对象的指针和`U`类型对象的指针，并将其应用在成员初始化列表中。这段代码隐含着一个要求："只有当存在从`U`指针对象到`T`指针对象的隐式转换时，这段代码才能通过编译"。换句话说，如果我们做出了上面问题中提到的操作，那么这段代码是无法通过编译的，我们不必到运行期程序莫名其妙结束了才去定位问题到底在哪。

另外需要一提的是，模板类中定义的成员函数模板（上面展示的是拷贝构造函数模板）不会影响编译器为你生成默认的成员函数。也就是说，即使我们提供了拷贝构造函数模板，默认情况下编译器仍然会为模板类生成的类实体生成一个默认的拷贝构造函数。

- 请使用成员函数模板生成"可接受所有兼容类型"的函数。
- 如果你声明成员模板用于"泛化拷贝构造"或者"泛化赋值操作"，你还是需要声明正常的拷贝构造函数和拷贝赋值运算符。
...
这是一个非常典型的有理数类的模板写法，并支持了乘法操作。我们进行这样的调用：
...
遗憾的是，这段代码无法通过编译。尽管它和条款24中的代码几乎一模一样，仅仅是将类和非成员函数`operator*`改写成了模板。

问题出在了调用`oneHalf * 2`之上，编译器会尝试在`Rational<int>`这个具体类中查找一个成员函数`operator*`，这个成员函数接受一个`int`作为参数，或者支持从`int`到`Rational<int>`的隐式转换（实际并不会，等会说原因）。同时，编译器也会查找一个非成员函数，接受一个`Rational<int>`和`int`参数，或者一个`Rational<int>`和一个`int`到`Rational<int>`的转换（实际也不会）。

从肉眼观察我们可以看到我们定义了非成员函数的`operator*`的模板，问题在于编译器不知道。我们要强调的一点是:<font color="red">编译器在模板实参推导过程中从不将隐式类型转换纳入考虑。</font>

也就是说，编译器不会考虑“是否有从`int`到`Rational<int>`的隐式类型转换”，因此不会尝试去寻找任何接受两个`Rational<int>`参数的`operator*`函数，而只会在"接受一个`Rational<int>`和一个`int`参数的范围内查找"。

回归本质，问题还是出在了"编译器不会为模板考虑隐式类型转换"，因此条款24中的非模板类的`operator*`函数就不担心编译器不会为其考虑隐式类型转换的问题了。

同样的，要解决这个问题，我们需要将模板转换为非模板，如何转换呢？其实模板进行了具现化，生成的具体类就不再是模板了。我们只需要将`operator*`与模板类一起进行具现化就好了。

一起进行具现化，首当其冲想到的就是将`operator*`作为成员函数，作为模板类的一部分。但是条款24已经告诉了我们将其作为成员函数带来的麻烦了，为此我们必须考虑"不将其作为成员函数，同时希望它和模板一起具现化"。

答案已经呼之欲出了，那就是让它成为模板类的友元。

成为友元之后，由于友元需要在类中进行声明。因此在生成`Rational<int>`类的时候，就会生成一个`const Rational<int> operator*(const Rational<int>& lhs, const Rational<int>& rhs)`的声明了。

但是还有一个小问题，定义呢？如果我们将这个函数的定义写在了类的外面，那么编译器无法找到它（这里暂时还不知道为什么）。因此我们在声明友元函数的同时，也要在模板类内为其提供定义（还记得吗，类内定义的成员函数自动成为内联）。

- 当我们编写一个类模板，而它所提供的“与此模板相关的”函数支持“所有参数的隐式类型转换”时，请将那些函数定义为“类模板内的友元函数”。

## 47. 请使用特征类（trait classes）表现类型信息

这一点比较...其实就是说如何为一个类表现其类型信息，具体看书。

- 特征类使得“类型相关信息”在编译期可用。它们以模板和模板特化完成实现。
- 整合重载技术后，特征类有可能在编译期间对类型执行`if...else`测试。

## 48. 认识模板元编程

看不懂。。

- 模板元编程可将工作由运行期移到编译期，因此得以实现早期错误侦测和更高的执行效率。
- 模板元编程可被用来生成“基于策略选择组合”的用户定制代码，也可以用来避免生成对某些特殊类型并不适合的代码。

## 49. 了解new-handler的行为

本条款介绍了一下当使用`operator new`时出现了内存分配失败问题时，`new-handler`如何处理这些问题，以及如何为类定制专属的`new-handler`。

具体内容需要看书，这里难以总结。

- `set_new_handler`允许用户指定一个函数，在内存分配无法获得满足时被调用。
- `nothrow`是一个颇为具现的工具，因为它只适用于内存分配，后继的构造函数调用仍然可能抛出异常。

## 50. 了解new和delete的合理替换时机

本条款介绍了一些需要自定义`new`和`delete`的时机，并说明自定义这些操作符的意义。

其中提到了一个细节上的概念，内存对齐。内存对齐我的个人理解大概就是：如果我们在分配内存时以所分配的变量的占用内存大小的整数倍（例如int占用4个字节）为起始地址的话，那么会大大的增加内存的访问速度。反之则会降低内存的访问速度。因此在自定义这些操作符的时候需要注意这一问题。

具体内容可以看书。

- 有许多理由需要自己定义`new`和`delete`，包括改善性能、对堆进行错误调试、以及收集堆的使用信息等。

## 51. 编写new和delete时需要固守常规

本条款算是对条款50的补充说明，同样需要看书。

- operator new应该内含一个无限循环，并在其中尝试分配内存，如果它无法满足内存需求，就应该调用`new-handler`。它也应该有能力处理0字节的内存申请。类专属版本还应该处理“比正确大小更大的错误申请”。
- operator delete应该在收到`nullptr`时不做任何事。类专属版本则还应该处理“比正确大小更大的错误申请”。
...
这一调用将调用标准的`operator new`和`Widget`的默认构造函数。

如果一切正常当然都好，如果`operator new`因为内存不足的问题抛出异常或许也能接受（因为我们很容易发现这这样的异常并处理），但是如果`operator new`分配内存成功了，但是在默认构造函数的过程中进行了动态内存分配呢？此时`operator new`并没有把指针的地址返回给`pw`，但是内存的确分配出去了，这就会造成内存泄漏。

但是这样的错误可以算得上是比较容易发生的，而且这一过程发生在分配内存的过程中，也就是运行时问题。因此这一问题的处理责任就落在了C++运行期系统的头上。

C++运行期系统默认的方式是：调用`operator new`所对应的`operator delete`，释放分配的内存。为什么说是“所对应的`operator delete`”？

标准形式下的`operator new`以及`operator delete`同样接收一个非类型参数`size_t`，这一参数指定了需要分配或释放的内存的大小。然而，如果用户定义了非标准形式的`operator new`呢（也许你想要记录日志，记录每次分配的内存大小，因此使用非标准形式的`new`来传递一个日志类的引用）？

答案也非常简单，如果你定义了一个非标准形式的`new`，则应该以相同的形式定义一个`delete`并在定义式中完成释放内存的操作，C++运行期系统会在`new`过程抛出异常时调用对应形式的`delete`。

- 当你写一个`placement operator new`，请确定也写出了对应的`placement operator delete`。如果没有这样做，你的程序可能发生隐藏极深且时断时续的内存泄漏。
- 当你声明`placement new`和`placement delete`，请确定不要无意识的遮掩了它们的正常版本。
...
- TR1自身只是一份规范。为了获得TR1提供的好处，你需要一份实物。一个好的来源是Boost。

## 55. 让自己熟悉Boost

 字面意思。。

- Boost是一个社群，也是一个网站。致力于免费、源码开放、同行复审的C++程序库开发。Boost在C++标准化过程中扮演了深具影响力的角色。
